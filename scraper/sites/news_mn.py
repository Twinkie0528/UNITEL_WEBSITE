...
IMAGE_SELECTORS = (
    "div.it-banner-slider-item img",
    "div.it-banner-slider-item video[poster]",
    "div[id*='banner'] img",
...
def scrape_news(output_dir: Path) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        logger.warning("news.mn fetch failed: %s", exc)
        return captures
...
def _extract_assets(
    ...
    *,
    ...
    for element in soup.select(selector):
        capture = _capture_image(element, page_url, out_dir, seen, index=starting_index + len(results))
        if capture:
