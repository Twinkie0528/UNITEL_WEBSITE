...
def _collect_imgs(page, output_dir, seen:Set[str], ads_only:bool, min_score:int) -> List[Dict]:
    ...
    for i in range(count):
        ...
        is_ad, score, reason = classify_ad("ikon.mn", src, landing, str(int(bbox["width"])), str(int(bbox["height"])), ("iframe" if tag=="iframe" else "onpage"), min_score=min_score)
        ...
        out.append({
            "site":"ikon.mn","src":src,"landing_url":landing,
...
def scrape_ikon(output_dir: str, dwell_seconds:int=45, headless:bool=True, ads_only:bool=True, min_score:int=2) -> List[Dict]:
