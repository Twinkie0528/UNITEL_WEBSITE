...
HOME = "https://gogo.mn"
BLOCK_PATTERNS = ("thumb", "thumbnail", "video-thumb", "story-card", "avatar", "logo", "icon", "/thumb/", "/cache/", "/icons/")
...
def _shot(output_dir: Path, src: str, index: int) -> str:
    fname = f"gogo_{int(time.time())}_{index}_{hashlib.md5(src.encode('utf-8', 'ignore')).hexdigest()[:8]}.png"
...
def _collect_candidates(page: Page, output_dir: Path, seen: Set[str]) -> List[Dict[str, Any]]:
    ...
    locator = page.locator(
        ...
        "section[class*='sponsor'] img, iframe[src*='ads'], iframe[id*='ad'], iframe[class*='ad']"
    )
    count = locator.count()
